...
The point of Robust Bayesian Statistics is to convince one's audience of their result by encapsulating the viewer's belief's in their baseline.
Which departs from the usual goal of statistics to efficiently as possible determine conclusion from the data. 
In short, the goal of Robust Bayesian Statistics is to be convincing rather than efficient.
...
The author uses the last 30 years of the S\&P 500\cite{SP500} as an example to illustrate the methods covered in this paper.
\pagebreak
\tableofcontents
\par
...
Integrating one gets $ \displaystyle\int_0^1 (1\!\m\! x)^2 dx =\eval{ \m \frac{1}{3}(1 \m x)^3 + C}_0^1=\frac{1}{3} $ therefore 
...
\begin{flalign*}
   ...
   \iff a(1\!\m\! x)\!\m\! bx=0 \therefore x=\frac{a}{a+b}\\
...
Additionally, suppose one person modeled $\Pr(\sigma=x) \propto \frac{1}{x}$ while a second person modeled standard deviation as 
$\Pr(\sigma=x)=x e^{\!\m  x}$.
...
the tree's standard deviation being a mile. If data is collected on the heights of Redwood trees and the posteriors for standard deviation disagree on parameters, 
...
by incorporating multiple reasonable priors and showing what these posteriors agrees on allows \textit{the data to speak for itself},
...
based off the last 30 years of the S\&P 500. The answer is surprisingly low between 52\%$-$55\%.
\par
...
\par
For example, suppose one rotated an object with \mbox{3 sections: \textit{T,S,H}} 
...
led to a small $\mathcal{R}(\theta)$ set then one has usable clear conclusions which were not the results of someone's prior 
...
\begin{flalign*}
   ...
   &\intertext{Variance and standard deviation are tricker, because they do not decrease monotonically with respect to $a$ or $b$. }
...
\begin{flalign*}
   ...
   &\int_{x}^{1}{B_f(y) dy} \le \int_{x}^{1}{B_t(y) dy}  \text{  for  }x \in [x_t,1] \\
...
\begin{flalign*}
   ...
   &\phantom{\ln((M+n)!)}\,=\ln(M!)+\ln(M^{n}+\frac{n(n\!\m\!1)}{2}M^{n\m1}+\ldots+n!)\\
   &\phantom{\ln((M+n)!)}\,=\ln(M!)+\ln(M^{n}\cdot\left(1+\frac{n(n\!\m\!1)}{2M}+\ldots+\frac{n!}{M^n}\right))\\[-.2em]
   &\ln((M+n)!)=\ln(M!)+n\ln(M)+\ln(1+\frac{n(n\!\m\!1)}{2M}+\ldots+\frac{n!}{M^n})\\[-.7em]
   &\lim_{M \to \infty}\ln(1+\frac{n(n\!\m\!1)}{2M}+\ldots+\frac{n!}{M^n})=0 \quad \text{therefore}\\
...
$d=\lambda\E[\sigma]=\lambda\sqrt{n}\E[\sqrt{p_s(1\!\m\!p_s)}]$. However, by doing this one is changing the hyperparameter, $d$, based on the data 
